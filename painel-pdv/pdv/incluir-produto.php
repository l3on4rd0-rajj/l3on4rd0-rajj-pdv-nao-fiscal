...
if(@count($res) > 0){
	...
	if($estoque < $quantidade){
		echo 'Produtos não disponivel em estoque!';
		exit();
	}

	// SE O PRODUTO JÁ FOI CONFERIDO UMA VEZ...
	$query_con2 = $pdo->query("SELECT * FROM itens_venda WHERE produto = '$id' AND venda = 0");
	...
	if(@count($res2) > 0){
		...
		$res2 = $pdo->prepare("UPDATE itens_venda SET quantidade = :quantidade, valor_total = :valor_total WHERE produto = '$id' AND venda = 0");
