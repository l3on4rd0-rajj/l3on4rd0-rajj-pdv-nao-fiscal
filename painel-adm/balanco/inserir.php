...
if(@count($res) > 0){
	...
	$query_con2 = $pdo->query("SELECT * FROM itens_balanco WHERE id_produto = '$id'");
	...
	if(@count($res2) > 0){
		...
		$res2 = $pdo->prepare("UPDATE itens_balanco SET qntd_conferida = :quantidade WHERE id_produto = '$id'");
		$res2->bindValue(":quantidade", $nova_quantidade);
		$res2->execute();

	} else{
		//INSERIR NA TABELA ITENS CHECK IN
		$res = $pdo->prepare("INSERT INTO itens_balanco SET id_produto = :id, nome = :nome, custo = :custo, qntd_conferida = :quantidade");
		$res->bindValue(":id", $id);
